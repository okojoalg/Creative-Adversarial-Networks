...
def cyclegan_arg_scope(instance_norm_center=True,
  ...
  with slim.arg_scope(
      ...
      weights_initializer=tf.compat.v1.random_normal_initializer(0, weights_init_stddev),
...
def cyclegan_upsample(net, num_outputs, stride, method='conv2d_transpose'):
  ...
  with tf.compat.v1.variable_scope('upconv'):
    ...
    if method == 'nn_upsample_conv':
      ...
      net = tf.pad(tensor=net, paddings=spatial_pad_1, mode='REFLECT')
      net = layers.conv2d(net, num_outputs, kernel_size=[3, 3], padding='valid')
    if method == 'bilinear_upsample_conv':
      net = tf.image.resize(
          net, [stride[0] * height, stride[1] * width], method=tf.image.ResizeMethod.BILINEAR)
      net = tf.pad(tensor=net, paddings=spatial_pad_1, mode='REFLECT')
      net = layers.conv2d(net, num_outputs, kernel_size=[3, 3], padding='valid')
