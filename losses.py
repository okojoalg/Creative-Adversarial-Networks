...
def CAN_loss(model):
    ...
    if model.experience_flag:
      try:
        model.experience_selection = tf.convert_to_tensor(value=random.sample(model.experience_buffer, 16))
      except ValueError:
        model.experience_selection = tf.convert_to_tensor(value=model.experience_buffer)
    ...
    correct_prediction = tf.equal(tf.argmax(input=model.y,axis=1), tf.argmax(input=model.D_c,axis=1))
    model.accuracy = tf.reduce_mean(input_tensor=tf.cast(correct_prediction, tf.float32))

    true_label = tf.random.uniform(tf.shape(input=model.D),.8, 1.2)
    false_label = tf.random.uniform(tf.shape(input=model.D_), 0.0, 0.3)

    model.d_loss_real = tf.reduce_mean(
      input_tensor=sigmoid_cross_entropy_with_logits(model.D_logits, true_label * tf.ones_like(model.D)))

    model.d_loss_fake = tf.reduce_mean(
      input_tensor=sigmoid_cross_entropy_with_logits(model.D_logits_, false_label * tf.ones_like(model.D_)))

    model.d_loss_class_real = tf.reduce_mean(
      input_tensor=tf.nn.softmax_cross_entropy_with_logits(logits=model.D_c_logits, labels=tf.stop_gradient(model.smoothing * model.y)))
    ...
    if model.style_net_checkpoint is None:
      model.g_loss_class_fake = tf.reduce_mean(
        input_tensor=tf.nn.softmax_cross_entropy_with_logits(logits=model.D_c_logits_,
          labels=tf.stop_gradient((1.0/model.y_dim)*tf.ones_like(model.D_c_))))
    else:
      model.classifier = model.make_style_net(model.G)
      model.g_loss_class_fake = tf.reduce_mean(
        input_tensor=tf.nn.softmax_cross_entropy_with_logits(logits=model.classifier,
          labels=tf.stop_gradient((1.0/model.y_dim)*tf.ones_like(model.D_c_))))

    model.g_loss_fake = -tf.reduce_mean(input_tensor=tf.math.log(model.D_))
    ...
    model.g_opt = tf.compat.v1.train.AdamOptimizer(learning_rate=model.learning_rate, beta1=0.5)
    model.d_opt = tf.compat.v1.train.AdamOptimizer(learning_rate=model.learning_rate, beta1=0.5)
...
def GAN_loss(model):
    ...
    model.g_loss = tf.reduce_mean(
      input_tensor=sigmoid_cross_entropy_with_logits(model.D_logits_, tf.ones_like(model.D_)))
...
def WGAN_loss(model):
    ...
    model.g_loss = -tf.reduce_mean(input_tensor=model.D_fake)
    model.wp= -tf.reduce_mean(input_tensor=model.D_fake) + tf.reduce_mean(input_tensor=model.D_real)
    ...
    model.slopes = tf.sqrt(tf.reduce_sum(input_tensor=tf.square(grad_D_x_hat), axis=[1,2,3]))
    model.gradient_penalty = tf.reduce_mean(input_tensor=(model.slopes - 1.) ** 2)
