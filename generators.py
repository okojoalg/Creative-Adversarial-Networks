...
def wgan_slim_cond(model, z, y, is_sampler=False):
    with tf.compat.v1.variable_scope("generator") as scope:
        ...
        z = concat([z,y],1)
        yb = tf.reshape(y, [-1, 1, 1, model.y_dim])
        z_ = linear(z, model.gf_dim*s_h8*s_w8*4, 'g_h0_lin')
        
        h0 = tf.reshape(z_, [-1, s_h8, s_w8, model.gf_dim*4]) 
        h0 = lrelu(model.g_bn0(h0, train=is_sampler))
        h0 = conv_cond_concat(h0, yb)

        h1 = model.upsample(h0, [-1, s_h4, s_w4, model.gf_dim*2], name='g_h1')
        h1 = lrelu(model.g_bn1(h1, train=is_sampler))
        h1 = conv_cond_concat(h1, yb)

        h2 = model.upsample(h1, [-1, s_h2, s_w2, model.gf_dim], name='g_h2')
        h2 = lrelu(model.g_bn2(h2, train=is_sampler))
        h2 = conv_cond_concat(h2, yb)

        h3 = model.upsample(h2, [-1, s_h, s_w, model.c_dim], name='g_h3')
...
def wgan_slim(model, z, is_sampler=False):
    with tf.compat.v1.variable_scope("generator") as scope:
        ...
        z_ = linear(z, model.gf_dim*s_h8*s_w8*4, 'g_h0_lin')
        z_ = tf.reshape(z_, [-1, s_h8, s_w8, model.gf_dim*4])

        h0 = model.upsample(
            z_, [-1, s_h8, s_w8, model.gf_dim*4], name='g_h0')
        h0 = lrelu(model.g_bn1(h1, train=is_sampler))

        h1 = model.upsample(
            h0, [-1, s_h4, s_w4, model.gf_dim*2], name='g_h1')
        h1 = lrelu(model.g_bn2(h2, train=is_sampler))

        h2 = model.upsample(
            h1, [-1, s_h2, s_w2, model.gf_dim], name='g_h2')
        h2 = lrelu(model.g_bn3(h3, train=is_sampler))

        h3 = model.upsample(
            h2, [-1, s_h, s_w, model.c_dim], name='g_h3')
